...
- Интерфейс ағылшын тілінде ашылады; KZ / RU / EN тілін жоғарғы оң жақтан ауыстырыңыз.
- Қызметкер экраны: `E0001` таңдаңыз → «Recommend next steps» → «Mark as done».
- Негіздемелер қызметкердің тілінде жазылады (`preferred_language`): `E0001` үшін қазақша, `E0002` үшін орысша.
...
![Қызметкер экраны: мақсатқа дайындық, дағдылар, факторлары бар ұсынылған қадамдар](docs/screens/employee_kk.png)

![HR экраны: команданың дайындығы, назар аударуды қажет ететін нәрселер, артта қалған дағдылар](docs/screens/hr_kk.png)

Мобильді нұсқа: қызметкер және HR:

<p>
<img src="docs/screens/mobile_employee_kk.png" alt="Мобильді нұсқа: қызметкер және HR (1)" width="260">
<img src="docs/screens/mobile_hr_kk.png" alt="Мобильді нұсқа: қызметкер және HR (2)" width="260">
</p>
...
- Негіздемелер қызметкердің тілінде (`preferred_language` бойынша kk / ru / en), интерфейс KZ / RU / EN тілдерінде; 60 дағдының, 40 белсенділіктің және 8 рөлдің атаулары қазақ және орыс тілдеріне аударылған (`web/i18n_names.json`, датасеттің өзі өзгермейді).
...
- **Кеңейтілген HR-дашборд** — бөлімше бойынша сүзгі және «каталог кемшіліктері»: қызметкерлерге мақсат үшін қажет, бірақ ешбір қолжетімді белсенділік көтере алмайтын дағдылар (мысалы, Leadership 39 қызметкерге қажет және 37 қызметкердің жоғарылауы үшін сыни, ал курс жоқ).
...
- **HR-ға арналған белсенділік конструкторы** — «каталог кемшіліктері» кестесінде «Белсенділік құру» батырмасы: жүйе бұғатталған қызметкерлердің рөлдері мен грейдтеріне сай жобаны толтырады (мысалы, Leadership — 39 адам), HR түзетіп, жариялайды, белсенділік бірден ұсынымдарға түседі, кемшілік жабылады.
...
AI кілті бапталған демо: https://kitan-a.com/careerquest/ — кейс деректері болғандықтан құпиясөзбен жабық.

| | |
|---|---|
| Бетке кіру (браузер терезесі) | логин `judge`, құпиясөз `drynsDhqCPsV` |
| Қосымшадағы HR экраны | құпиясөз `hr-demo` |

Бір басумен ашылатын сілтемелер (логин сілтемеде бар; браузер бәрібір сұраса — `judge` / `drynsDhqCPsV`):
- `E0025` қызметкері, қазақша интерфейс: https://judge:drynsDhqCPsV@kitan-a.com/careerquest/?employee=E0025&lang=kk
- `E0001` қызметкері (тексеру бөліміндегі сценарий): https://judge:drynsDhqCPsV@kitan-a.com/careerquest/?employee=E0001
- HR экраны, кіру орындалған: https://judge:drynsDhqCPsV@kitan-a.com/careerquest/?view=hr&hr=hr-demo&lang=kk

Дәл осы параметрлер жергілікті түрде де жұмыс істейді, мысалы http://127.0.0.1:8000/?view=hr&hr=hr-demo. Ашылғаннан кейін олар мекенжай жолынан алынып тасталады.

Демо осы репозиторийдің соңғы коммитінде жұмыс істейді және датасеттің таза деректерімен қайта іске қосылады; жүктеулер мен «аяқталды» белгілері қайта іске қосылғанға дейін сақталады. Біздің есептік жазбаларымызсыз өз бетінше тексеру үшін `./run.sh` (ережелер режимі) немесе өз `OPENAI_API_KEY` кілтіңіз жеткілікті.
